...
tableBody.addEventListener("click", (event) => {
  ...
  if (event.target.classList.contains("done-button")) {
    ...
    let parentTask = event.target.closest("section");
  ...
  if (event.target.classList.contains("edit-button")) {
    ...
    let editTaskName = document.querySelector(`#inputtask-${taskId}`);
    let editDesc = document.querySelector(`#textarea-${taskId}`);
    let editAssign = document.querySelector(`#assign-${taskId}`);
    let editDate = document.querySelector(`#duedate-${taskId}`);
    let editStatus = document.querySelector(`#status-${taskId}`);

    let editNameVal = editTaskName.value;
    let editDescVal = editDesc.value;
    let editDateVal = editDate.value;
    let editAssignVal = editAssign.value;
    let editStatusVal = editStatus.value;
    // Update the task
